...
function run_sim_patterns(; nvals)
    ...
    p = (
        ...
        aS = 1.5,
        aI = 1.3,
        AcritS = 0.3/(1.5*(0.33-0.53*0.3)), # D/(aS*(e-h*D))
        AcritI = 0.3/(1.3*(0.33-0.53*0.3))
    )
    ######################
...
@info "Run simulations for figure S8, part 2"
sim_result = run_sim_patterns(; nvals)
jldsave("simulation_results/S8_pattern.jld2"; sim_result...)
