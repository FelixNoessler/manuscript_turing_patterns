...
function env_heterogeneity_I_with_plastic_dispersal!(du,u,p,t)
    ### states
    @unpack Nx_star, Ny_star, Ax_star, Ay_star, Sx_star, Sy_star,
            Nx, Ny, Ax, Ay, Sx, Sy, Ix, Iy = u

    ### parameters
    @unpack S, D, r, h, e, dN, dA, dS, kI, dmaxI, aS, aI = p

    ### equantions of first system
    gxS_star = aS*Ax_star/(1+aS*h*Ax_star)
    gyS_star = aS*Ay_star/(1+aS*h*Ay_star)

    du.Nx_star = D*S -D*Nx_star -r*Ax_star*Nx_star +dN*(Ny_star-Nx_star)
    du.Ny_star = D*S -D*Ny_star -r*Ay_star*Ny_star +dN*(Nx_star-Ny_star)
    du.Ax_star = r*Ax_star*Nx_star -gxS_star*Sx_star -D*Ax_star +dA*(Ay_star-Ax_star)
    du.Ay_star = r*Ay_star*Ny_star -gyS_star*Sy_star -D*Ay_star +dA*(Ax_star-Ay_star)
    du.Sx_star = e*gxS_star*Sx_star -D*Sx_star +dS*(Sy_star-Sx_star)
    du.Sy_star = e*gyS_star*Sy_star -D*Sy_star +dS*(Sx_star-Sy_star)

    ### calculation of the supply concentration
    Seff_x = S +dN*(Ny_star-Nx_star)/D
    Seff_y = S +dN*(Nx_star-Ny_star)/D

    ### equations of second system
    gxS = aS*Ax/(1+aS*h*Ax)
    gyS = aS*Ay/(1+aS*h*Ay)
    gxI = aI*Ax/(1+aI*h*Ax)
    gyI = aI*Ay/(1+aI*h*Ay)
    ...
    du.Nx = D*Seff_x -r*Ax*Nx -D*Nx
    du.Ny = D*Seff_y -r*Ay*Ny -D*Ny
...
function run_sim_patterns(; nvals)
    ...
    @time for s in eachindex(dS_vals)
        ...
        u0_final = ca.ComponentArray(;
            ...
            Sx_star=u_onlyS.Sx_star[end],
            Sy_star=u_onlyS.Sy_star[end],
        ...
        @Threads.threads for i in eachindex(dmaxI_vals)
            ...
            prob = ODEProblem(
                env_heterogeneity_I_with_plastic_dispersal!,
                u0_final,
                tspan,
                p_final;
                callback=cb)
...
@info "Run simulations for figure S7 part 1"
sim_result = run_sim_patterns(; nvals)
jldsave("simulation_results/S7_I_with_plastic_dispersal_env_het.jld2"; sim_result...)
